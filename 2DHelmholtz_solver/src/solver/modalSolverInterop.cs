    ...
    public static class modalSolverInterop
    {
        ...
        public static bool Initialize(string dllPath = null)
        {
            try
            {
                ...
                IntPtr procAddress = GetProcAddress(_dllHandle, "solve_modalspectralanalysisCPP");
                if (procAddress == IntPtr.Zero)
                {
                    int errorCode = Marshal.GetLastWin32Error();
                    _lastError = $"Exported function 'solve_modalspectralanalysisCPP' not found in DLL. Error code: {errorCode}";
        ...
        private static string GetDllPath()
        {
            ...
            {
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "modalspectral_solverCPP.dll"),
            Path.Combine(Environment.CurrentDirectory, "modalspectral_solverCPP.dll"),
            Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "modalspectral_solverCPP.dll")
            };
