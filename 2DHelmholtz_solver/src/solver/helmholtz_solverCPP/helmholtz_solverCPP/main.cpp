...
void print_matrices(helmholtz2d_spectral_solver& helmholtz_spec_solver, stopwatch_events& stopwatch)
{
	...
	if (print_matrices == true)
	{
		...
		std::cout << "Printing matrices to file at " << stopwatch_elapsed_str.str() << " secs" << std::endl;
		...
		if (node_map_file.is_open())
		{
			...
			for (const auto& nd_map : helmholtz_spec_solver.nodeid_map)
			{
		...
		if (node_file.is_open())
		{
			...
			for (const auto& spec_node_m : helmholtz_spec_solver.spec_mesh2d.spectral_node_list)
			{
		...
		if (helmholtz_spec_solver.global_system_matrix.nonZeros() > 0)
		{
			std::ofstream matrix_file(debug_dir + "/global_system_matrix.csv");

			if (matrix_file.is_open())
			{
				using MatrixType = std::decay<decltype(helmholtz_spec_solver.global_system_matrix)>::type;
				...
				if (is_complex) 
				{
					...
					Eigen::MatrixXcd dense = Eigen::MatrixXcd(helmholtz_spec_solver.global_system_matrix);
		...
		if (helmholtz_spec_solver.global_field_vector.size() > 0)
		{
			std::ofstream field_file(debug_dir + "/global_field_vector.txt");
			if (field_file.is_open())
			{
				field_file << "Index,Value\n";
				for (int i = 0; i < helmholtz_spec_solver.global_field_vector.size(); ++i)
				{
					field_file << i << "," << helmholtz_spec_solver.global_field_vector(i) << "\n";
				}
				field_file.close();
		...
		if (helmholtz_spec_solver.global_dirichlet_BC_flags_vector.size() > 0)
		{
			std::ofstream bc_file(debug_dir + "/dirichlet_bc_flags.txt");
			if (bc_file.is_open())
			{
				bc_file << "Index,IsDirichlet\n";
				for (int i = 0; i < helmholtz_spec_solver.global_dirichlet_BC_flags_vector.size(); ++i)
				{
					bc_file << i << "," << helmholtz_spec_solver.global_dirichlet_BC_flags_vector(i) << "\n";
				}
				bc_file.close();
		...
		if (helmholtz_spec_solver.K_ff.nonZeros() > 0)
		{
			std::ofstream matrix_file(debug_dir + "/K_ff.csv");

			if (matrix_file.is_open())
			{
				using MatrixType = std::decay<decltype(helmholtz_spec_solver.K_ff)>::type;
				using Scalar = typename MatrixType::Scalar;
				bool is_complex = std::is_same<Scalar, std::complex<double>>::value;
				...
				{
					// Convert to dense and print
					Eigen::MatrixXcd dense = Eigen::MatrixXcd(helmholtz_spec_solver.K_ff);
					matrix_file << "Real Part:\n";
					matrix_file << dense.real() << "\n\n";
					matrix_file << "Imaginary Part:\n";
					matrix_file << dense.imag() << "\n";
		...
		if (helmholtz_spec_solver.F_f.size() > 0)
		{
			std::ofstream ff_file(debug_dir + "/F_f.txt");
			if (ff_file.is_open())
			{
				ff_file << "Index,Value\n";
				for (int i = 0; i < helmholtz_spec_solver.F_f.size(); ++i)
				{
					ff_file << i << "," << helmholtz_spec_solver.F_f(i) << "\n";
				}
				ff_file.close();
		...
		if (summary_file.is_open())
		{
			...
			summary_file << "Global system matrix: "
				<< helmholtz_spec_solver.global_system_matrix.rows() << "x"
				<< helmholtz_spec_solver.global_system_matrix.cols()
				<< ", non-zeros: " << helmholtz_spec_solver.global_system_matrix.nonZeros() << "\n";
			summary_file << "Global field vector size: " << helmholtz_spec_solver.global_field_vector.size() << "\n";
			summary_file << "Dirichlet BC flags size: " << helmholtz_spec_solver.global_dirichlet_BC_flags_vector.size() << "\n";
			summary_file << "K_ff matrix: " << helmholtz_spec_solver.K_ff.rows() << "x" << helmholtz_spec_solver.K_ff.cols() << "\n";
			summary_file << "F_f vector size: " << helmholtz_spec_solver.F_f.size() << "\n";
			summary_file << "Number of spectral nodes: " << helmholtz_spec_solver.spec_mesh2d.spectral_node_list.size() << "\n";
			summary_file.close();
